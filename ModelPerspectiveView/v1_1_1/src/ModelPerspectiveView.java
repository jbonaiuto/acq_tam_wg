...
public class ModelPerspectiveView extends NslJavaModule implements KeyListener{
...
public void init(Nsl3dCanvas mc)
{
	...
	viewPosition= new  javax.vecmath.Vector3d(1, 5, 1);
...
protected void process()
{
	...
	lastDist=distance;
	efferenceCopyOut.set(__tempacq_tam_wg_ModelPerspectiveView_v1_1_1_src_ModelPerspectiveView0.setReference(NslElemMult.eval(__tempacq_tam_wg_ModelPerspectiveView_v1_1_1_src_ModelPerspectiveView0.get(), delta, efferenceCopyOut)));
	if(distance<.1&&goSignal.get()>actionThreshold&&system.getCurrentTime()>=(arrivalTime+.1))
	{
		...
		goalX=NslMax.eval(-mainCanvas.getMazeDim().getX()+6,NslMin.eval(mainCanvas.getMazeDim().getX()-6,targetPosition.get(0)));
		...
		goalY=NslMax.eval(-mainCanvas.getMazeDim().getZ()+6,NslMin.eval(mainCanvas.getMazeDim().getZ()-6,targetPosition.get(1)));
...
{
	if(system.getCurrentTime()>system.getDelta()&&NslOperator.distance.eval(currentWorldPosition.get(),goalPosition.get())>.01)
	{
		...
		else if(goalOrientation.get()>currentOrientation.get()&&(__tempacq_tam_wg_ModelPerspectiveView_v1_1_1_src_ModelPerspectiveView2.setReference(__tempacq_tam_wg_ModelPerspectiveView_v1_1_1_src_ModelPerspectiveView1.setReference(Math.PI*2-goalOrientation.get()).get()+currentOrientation.get())).get()<(goalOrientation.get()-currentOrientation.get()))
	...
	if(system.getCurrentTime()>system.getDelta()&&NslOperator.abs.eval(getRelativeAngle(currentOrientation.get(),goalOrientation.get()))<.01)
	{
...
public void makeConn(){
	...
	/* Temporary variables */
		NslDouble1 __tempacq_tam_wg_ModelPerspectiveView_v1_1_1_src_ModelPerspectiveView0 = new NslDouble1(1);
		NslDouble0 __tempacq_tam_wg_ModelPerspectiveView_v1_1_1_src_ModelPerspectiveView1 = new NslDouble0();
		NslDouble0 __tempacq_tam_wg_ModelPerspectiveView_v1_1_1_src_ModelPerspectiveView2 = new NslDouble0();

	/* GENERIC CONSTRUCTOR: */
	public ModelPerspectiveView(String nslName, NslModule nslParent, int size, double actionThreshold)
{
		super(nslName, nslParent);
		this.size=size;
		this.actionThreshold=actionThreshold;
		initSys();
		makeInstModelPerspectiveView(nslName, nslParent, size, actionThreshold);
	}

	public void makeInstModelPerspectiveView(String nslName, NslModule nslParent, int size, double actionThreshold)
{ 
